import { Address, getContractEvents, parseEventLogs, prepareContractCall, prepareEvent, readContract, sendTransaction, waitForReceipt } from "thirdweb";
...
export const createCollection = async (name: string, uri: string, account: Account) => {
    ...
    const createCollectionEvent = prepareEvent({
        contract,
        signature: "event CreateCollection(string name, string uri, address curator, uint256 id)"
    });
...
export const listAsset = async ({ collectionId, tokenURI, price, royaltyWallet, royaltyPercentage, paymentSplits, account }: IListAsset) => {
    const transaction = await prepareContractCall({
        contract,
        method: "function listAsset(uint256 collectionId, string tokenURI, uint256 price, address royaltyWallet, uint256 royaltyPercentage, (address paymentWallet, uint256 paymentPercentage)[] paymentSplits)",
        params: [BigInt(collectionId), tokenURI, BigInt(price), royaltyWallet, BigInt(royaltyPercentage), paymentSplits]
    });
