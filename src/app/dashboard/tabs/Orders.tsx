        ...
        <div className="flex flex-col gap-y-4 px-4">
            <div className="flex gap-x-4 my-4">
                <div className="flex gap-x-2 items-center border-2 rounded-xl px-2 w-full lg:w-[90%]">
                    ...
                    <input 
                    onChange={(e: any) => setFilters({...filters, searchInput: e.target.value})}
                    placeholder="Search by name or trait..." className="w-full bg-transparent border-none outline-none focus:outline-none" />
                </div>
            ...
            <Table>
                ...
                <TableBody>
                    {orders.length > 0 ? orders.map((item: any, index) => (
                        <TableRow key={index}>
                            <TableCell>{item._id}</TableCell>
                            <TableCell className="font-medium">{item.name}</TableCell>
                            <TableCell>{item?.saleId?.ItemPurchasedOn ? moment(item?.saleId?.ItemPurchasedOn).format('DD-MM-YY') : '-/-'}</TableCell>
                            <TableCell>{moment().diff(moment(item?.saleId?.ItemPurchasedOn))}</TableCell>
