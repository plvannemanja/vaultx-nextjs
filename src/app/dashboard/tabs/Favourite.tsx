...
export default function Favourite() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      let curation: any =
        await favouriteService.getUserLikedCollections(filters);

      nfts = nfts.data.nfts.map((item: any) => {
        return item.nftId;
      });

      curation = await Promise.all(
        curation.data.curations.map(async (item: any) => {
          const info = await collectionServices.getCollectionById(
  ...
  return (
    <div className="flex flex-col gap-y-4 px-4">
      <div className="flex gap-x-4 my-4">
        <SearchWithDropDown
          setState={(e) => setFilters({ ...filters, filters: e })}
        />
      </div>
      ...
      {tab === 'curation' && (
        <div className="flex gap-4 flex-wrap my-4 justify-center md:justify-start">
          {data.curation.length > 0
            ? data.curation.map((collection: any, index: number) => {
                return (
      ...
      {tab === 'artist' && (
        <div className="flex gap-4 flex-wrap my-4 justify-center md:justify-start">
          {data.artist.length > 0
            ? data.artist.map((item: any, index: number) => {
                return (
                  <div className="w-[23rem]" key={index}>
                    <ArtistsCard
                      image={item.image}
