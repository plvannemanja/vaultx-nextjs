...
export default function BasicDetails({ handler, nextStep }: { handler: (data: any, error: any) => void, nextStep: (next?: boolean) => void }) {
    ...
    const handleAttachment = (file: any, index: number) => {
        ...
        if (attachment.size < maxFileSize && acceptedFormats.includes(`.${fileExtension}`)) {
            ...
            <div className="flex gap-y-5 flex-col lg:flex-row lg:justify-between">
                ...
                <div className='flex flex-col gap-y-5 lg:w-[55%]'>
                    ...
                    <div className="flex flex-col gap-y-2">
                        ...
                        <Textarea value={formData.productDescription ? formData.productDescription : ''} onChange={(e) => setFormData({ ...formData, productDescription: (e.target as any).value })} className="w-full border-none bg-[#161616]" placeholder="Please describe your product" />
                    </div>
                    ...
                    <div className="flex flex-col gap-y-2">
                        ...
                        <Input value={formData.artistName ? formData.artistName : ''} onChange={(e) => setFormData({ ...formData, artistName: (e.target as any).value })} className="w-full border-none bg-[#161616]" type="text" placeholder="Enter Collection Name" />
                    </div>
